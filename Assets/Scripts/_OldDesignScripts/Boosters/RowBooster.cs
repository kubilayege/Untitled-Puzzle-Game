...
class RowBooster : Booster
{
    ...
    public void ReleaseHoldingBooster()
    {
        ...
        {
            RowBooster r = GameManager.holdingBooster.GetComponent<RowBooster>();
            if (r == null)
                return;
        }
        else
    ...
    public void TryHoldingBooster()
    {
        ...
        {
            if (hit.transform.TryGetComponent<RowBooster>(out RowBooster booster))
            {
    ...
    public override void Place(CellManager.Pos pos)
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            Tuple<int, int> delP = new Tuple<int, int>(i, pos.y);
